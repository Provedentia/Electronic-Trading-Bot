...
def on_startup(state_manager):
    state_manager.send_order('XLF','BUY', 999, 10 )
    pass
...
def XLF_position(XLF_price: int, BOND_price: int, GS_price: int,  MS_price: int, WFC_price: int):
    if (10 * XLF_price) > ((3 * BOND_price) + (2 * GS_price) + (3 * MS_price) + (3 * WFC_price) + 100):
           ExchangeConnection.send_add_message()
def main():
    ...
    while True:
        ...
        state_manager.send_order("BOND","BUY",999,"Any")
        state_manager.send_order("BOND","SELL",1001,"Any")
