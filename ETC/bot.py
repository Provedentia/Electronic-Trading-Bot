...
def on_startup(state_manager):
    ...
    state_manager.send_order('BOND','SELL', 1001, 10)
    pass

def on_book(state_manager, book_message):
    """Called whenever the book for a symbol updates."""

    sym = book_message['symbol']
    buy = book_message['buy']
    sell = book_message['sell']
    ...
    if sym == 'BOND':
        ...
        for price, size in buy:
            if price > fair:
                state_manager.send_order(sym, Dir.SELL, price, size)
                adjust_last_buy(sym, price)
        for price, size in sell:
            if price < fair:
                state_manager.send_order(sym, Dir.BUY, price, size)
                adjust_last_sell(sym, price)
    if sym == 'VALE' :
        x = buy[0]
        y= sell[0]
        adjust_last_buy(sym, x[0])
        adjust_last_sell(sym, y[0])
        fair = fair_VAL()
        for price, size in buy:
            if price > fair:
                state_manager.send_order(sym, Dir.SELL, price, size)
...
def on_trade(state_manager, trade_message):
    sym = trade_message['symbol']
    price = trade_message['price']
    adjust_last_price(sym,price)
    
def adjust_last_buy(sym: str, price: int):
    last_sell_dict[sym] = price

def adjust_last_price(sym: str, price: int):
    last_price_dict[sym] = price

def call_last_price(sym: str):
    return last_price_dict[sym]

def call_last_buy(sym: str):
    return last_sell_dict[sym]

def adjust_last_sell(sym: str, price: int):
    last_sell_dict[sym] = price

def call_last_sell(sym: str):
    return last_sell_dict[sym]

def fair_VAL(): 
    a = call_last_buy('VALBZ')
    c = call_last_buy('VALE')
    b = call_last_sell('VALBZ')
    d = call_last_sell('VALE')
    if a >= b:
        fair = (abs(a - b) / 2) + b
    else:
        fair = (abs(a - b) / 2) + a
    if c >= d:
        ...
        fair_2 = (abs(c - d) / 2) + c
    if fair_2 > fair:
        final_fair = ((fair_2 - fair) / 2) + fair
    else:
        final_fair = ((fair - fair_2) / 2) + fair
    return final_fair
...
def fair_XLF():
    ...
    d = call_last_sell('MS')
    e = call_last_buy('WFC')
    f = call_last_sell('WFC')
    fair = 0
    if a > 0 and b > 0 and c > 0 and d > 0 and e > 0 and f > 0:
        fair = ((3 * 1000) + ((a + b) + (e + f) + (3 * (c + d) / 2))) / 10
    return fair
